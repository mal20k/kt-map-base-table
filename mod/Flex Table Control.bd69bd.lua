...
function click_ITDSize()
        getObjectFromGUID("4ee1f2").setScale({x=0.770468831, y=1.0, z=0.663833141})
end

function click_OpenSize()
        getObjectFromGUID("4ee1f2").setScale({x=0.83, y=1.0, z=0.61})
end

function click_applySurface(_, color, alt)
    if permissionCheck(color) then
        if not alt then
            updateSurface()
            broadcastToAll("New MAT Image Applied", {0.2,0.9,0.2})
        else
            --updateSurfaceTable()
...
function click_applySurfaceTable(_, color, alt)
    if permissionCheck(color) then
            updateSurfaceTable()
            broadcastToAll("New TABLE Image Applied", {0.2,0.9,0.2})
    end
...
--Loads table surface
function click_loadMemory(_, color, index, alt)
    if permissionCheck(color) then
...
function updateSurfaceTable()
    local customInfo = obj_surface2.getCustomObject()
    if self.getInputs()[2].value == "" then return end
    customInfo.image = self.getInputs()[2].value
    obj_surface2.setCustomObject(customInfo)
    obj_surface2 = obj_surface2.reload()
end
...
function createOpenCloseButton()
    ...
    self.createButton({
        ...
        position={0,0.2,3.1}, rotation={-45,0,0}, height=600, width=600, scale={0.7,0.7,0.7},
...
function createSurfaceButtons()
    ...
    self.createButton({
        ...
        position={0,0,4}, height=0, width=0, font_size=300, font_color={1,1,1}
    })
    --Functional
    local btn_width=1000
    local btn_font = 150
    self.createButton({
        label="Apply Image\nTo Mat", click_function="click_applySurface",
        function_owner=self, tooltip="Apply URL as table image",
        position={0,0,7}, height=440, width=btn_width, font_size=btn_font,
    })
    self.createButton({
        label="ITD Size", click_function="click_ITDSize",
        function_owner=self, tooltip="Apply URL as table image",
        position={-8,0,7}, height=440, width=btn_width, font_size=btn_font,
    })
    self.createButton({
        label="Open Size", click_function="click_OpenSize",
        function_owner=self, tooltip="Apply URL as table image",
        position={-8,0,9}, height=440, width=btn_width, font_size=btn_font,
    })
    self.createButton({
        label="Apply Image\nTo Table", click_function="click_applySurfaceTable",
        function_owner=self, tooltip="Apply URL as table image",
        position={3,0,7}, height=440, width=btn_width, font_size=btn_font,
    })
    self.createButton({
    ...
    --Label
    self.createButton({
        label="Load From Memory\nLeft click: Mat / Right click: Table", click_function="none",
        position={0,0,8.3}, height=0, width=0, font_size=250, font_color={1,1,1}
    })
    ...
    for i, memoryEntry in ipairs(tableImageData) do
        ...
        self.createButton({
            ...
            position={3.6,0,9.5+0.5*(i-1)}, height=240, width=600,
